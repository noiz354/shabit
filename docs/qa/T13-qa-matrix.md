...
| Spec 17 range picker | Sheet rentang (app bar 📅) | `range_changed{preset,days,module}`, `range_custom_applied{days}` | `src/range.js` (`openSheet`, `validateCustomRange`) | `tests/detail-range.test.js` (preset aria-pressed; dari>sampai → error inline `role=alert`, **`alert()` tidak dipanggil** — sebelumnya 3× alert; kustom valid → setRange + event + hash; preset 7H → event) | **PASS** (DOM); `range_empty_shown` belum dipicu (OPEN) |
